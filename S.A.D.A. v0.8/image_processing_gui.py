...
class ImageProcessingGUI:
    ...
    def update_anomalies(self, *args):
        ...
        if self.parent.image_selection.polygon_points:
            ...
            left = max(int(left - x_offset), 0)
            right = min(int(right - x_offset), img_width)
            top = max(int(top - y_offset), 0)
            bottom = min(int(bottom - y_offset), img_height)
    ...
    def analyze_image(self):
        ...
        if self.parent.image_selection.polygon_points:
            ...
            left = max(int(left - self.parent.x_offset), 0)
            right = min(int(right - self.parent.x_offset), img_width)
            top = max(int(top - self.parent.y_offset), 0)
            bottom = min(int(bottom - self.parent.y_offset), img_height)
            ...
            self.display_image_on_canvas(self.parent.image)
            self.parent.zoomed_selection_coords = (left, top, right, bottom)  # Save the selection coordinates
        else:
            image_array = np.array(self.parent.original_image)
            anomalies = find_anomalies(image_array, threshold, self.parent.anomaly_type.get(), self.parent.method.get())
            highlighted_image = highlight_anomalies(image_array, anomalies, color)
        ...
        self.parent.analyzed_image = self.parent.image.copy()  # Aggiorna l'immagine analizzata
    ...
    def zoom_to_selection(self):
        if self.parent.image_selection.polygon_points:
            ...
            # Adjust the coordinates to account for the offset and ensure they are within bounds
            left = max(int(left - self.parent.x_offset), 0)
            right = min(int(right - self.parent.x_offset), self.parent.original_image.width)
            top = max(int(top - self.parent.y_offset), 0)
            bottom = min(int(bottom - self.parent.y_offset), self.parent.original_image.height)

...
from config import IMAGE_MAX_SIZE, COLOR_MAP, LOGO_PATH
...
class ImageAnalyzer:
    ...
    def load_image(self):
        ...
        try:
            ...
            self.image.thumbnail(IMAGE_MAX_SIZE, Image.LANCZOS)
    ...
    def display_image_on_canvas(self, image):
        ...
        self.canvas.create_image((self.canvas.winfo_width() - image.width) // 2, (self.canvas.winfo_height() - image.height) // 2, anchor=tk.NW, image=self.tk_image)
    ...
    def analyze_image(self):
        ...
        if self.polygon_points:
            ...
            mask = create_mask(self.original_image.width, self.original_image.height, self.polygon_points)
            ...
            anomalies = find_anomalies(Image.fromarray(masked_image), threshold)

            # Creare un'immagine per evidenziare le anomalie solo all'interno del poligono
            highlighted_image = image_array.copy()
            highlighted_image[anomalies] = COLOR_MAP[color]  # Usa il colore selezionato
            ...
            anomalies = find_anomalies(self.image, threshold)
            highlighted_image = highlight_anomalies(self.image, anomalies, color)

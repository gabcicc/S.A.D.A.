...
class ImageSelection:
    def __init__(self, parent):
        ...
        self.start_x = 0
        self.start_y = 0
        self.image = None  # Aggiungi l'attributo per memorizzare l'immagine
    ...
    def load_image(self):
        ...
        try:
            self.image = Image.open(self.parent.image_path)  # Memorizza l'immagine in self.image
            if self.image.mode == 'RGBA':
                self.image = self.image.convert('RGB')

            max_size = (800, 600)
            self.image.thumbnail(max_size, Image.LANCZOS)
            self.parent.original_image = self.image.copy()  # Mantieni l'originale per eventuali reset

            self.parent.image_processing_gui.display_image_on_canvas(self.image)
